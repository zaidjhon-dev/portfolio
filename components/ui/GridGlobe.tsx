...
const GridGlobe = () => {
  ...
  useEffect(() => {
    if (!mounted || !activeHub) return;
    const tick = () => {
      const now = new Date();
      const utc = now.getTime() + now.getTimezoneOffset() * 60000;
      const hubDate = new Date(utc + 3600000 * activeHub.offsetHours);
      setCurrentTime(
        hubDate.toLocaleTimeString("en-US", {
          hour: "2-digit",
          minute: "2-digit",
          second: "2-digit",
          hour12: true,
  ...
  return (
    <div className="relative w-full h-full min-h-[220px] sm:min-h-[260px] flex flex-col justify-between overflow-hidden select-none">
      ...
      {/* Visual SVG layer — pointer-events off */}
      <svg
        viewBox="0 0 800 400"
        preserveAspectRatio="xMidYMid meet"
        className="w-full h-auto max-h-[240px] pointer-events-none drop-shadow-[0_0_12px_rgba(56,189,248,0.15)]"
        aria-hidden="true"
      >
        <defs>
        ...
        {ARCS.map((arc, i) => {
          ...
          return (
            <g key={i}>
              <path d={pathD} fill="none" stroke="rgba(203,172,249,0.25)" strokeWidth="1.5" strokeDasharray="4 4" />
              <path
                d={pathD}
                fill="none"
                stroke="url(#arc-grad)"
                ...
                style={{ strokeDasharray: "30 150", animationDuration: `${3.5 + i * 0.8}s` }}
              />
            </g>
          );
        })}
        ...
        {GLOBAL_HUBS.map((hub) => {
          ...
          return (
            <g key={hub.id}>
              {hub.isPrimary && (
                <>
                  <circle cx={hub.cx} cy={hub.cy} fill="none" stroke="#cbacf9" strokeWidth="1.2">
                  ...
                  <circle cx={hub.cx} cy={hub.cy} fill="none" stroke="#38bdf8" strokeWidth="0.8">
              ...
              {isSelected && !hub.isPrimary && (
                <circle cx={hub.cx} cy={hub.cy} r="12" fill="none" stroke="#38bdf8" strokeWidth="1.5" className="animate-pulse" />
              )}
              <circle
                cx={hub.cx}
                cy={hub.cy}
      ...
      {/* ── Clickable hit-target SVG (same viewBox/sizing as visual SVG) ── */}
      <svg
        viewBox="0 0 800 400"
        preserveAspectRatio="xMidYMid meet"
        className="absolute inset-0 w-full h-auto max-h-[240px] m-auto"
        ...
        <AnimatePresence mode="wait">
          {activeHub && (
            <motion.div
              ...
              className="flex items-center gap-1.5 text-[11px] sm:text-xs text-neutral-300
                         bg-black/50 backdrop-blur-sm border border-white/10
                         rounded-full px-3 py-1.5 shadow-lg pointer-events-auto"
            >
              <IoGlobeOutline className="w-3.5 h-3.5 text-purple flex-shrink-0" />
              ...
              <span className="text-neutral-500 mx-0.5">·</span>
              <IoTimeOutline className="w-3 h-3 text-purple flex-shrink-0" />
